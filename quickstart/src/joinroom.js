...
const $muteAudioButton = $('#muteAudio');
const $muteVideoButton = $('#stopVideo');
const helper = require('./helper');
const createScreenTrack = helper.createScreenTrack;
...
async function joinRoom(token, connectOptions) {
  ...
  // Inside your `joinRoom` function, add event handlers for the buttons:
  $muteVideoButton.click(() => {
  if (room.localParticipant) {
    const localVideoTrack = room.localParticipant.videoTracks.values().next().value.track;
    if (localVideoTrack) {
      isVideoMuted = !isVideoMuted; // Toggle the mute state
  ...
  if (room.localParticipant) {
    ...
    if (localAudioTrack) {
      isAudioMuted = !isAudioMuted; // Toggle the mute state
  ...
  try {
    ...
    screenTrack.on('stopped', () => {
      if (room) {
        ...
        screenTrack.stop();
        screenTrack.detach(screenPreview);
        //screenTrack = null;
            }
      // Detach screenTrack from screen preview
    });
